...
std::tuple<at::Tensor, at::Tensor> set_params_splitkv(Flash_fwd_params &params, const int batch_size,
    ...
    if (p_dropout == 0.0f) {  // SplitKV is not implemented for dropout
        ...
        if (params.num_splits > 1) {
            softmax_lse_accum = torch::zeros({batch_size, num_heads, max_seqlen_q}, opts.dtype(at::kFloat));
            out_accum = torch::zeros({batch_size, num_heads, max_seqlen_q, head_size_rounded}, opts.dtype(at::kFloat));
